"""
Training script for CFG Diffusion Model

Trains the U-Net with classifier-free guidance (10% label dropout) on MNIST.
The trained model supports both conditional and unconditional generation
at inference time via a single null token (label=10).
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train CFG Diffusion Model')
    ...
    print("=" * 80)
    print("DIFFUSION MODEL TRAINING  (CFG)")
    print("=" * 80)
    ...
    try:
        ...
        plt.title('Training & Validation Loss - CFG Diffusion Model')
        plt.legend()
        plt.grid(True)
        plt.savefig('training_loss_cfg.png', dpi=150, bbox_inches='tight')
        print("Loss curve saved: training_loss_cfg.png")
